...
use rustc_middle::middle::codegen_fn_attrs::ferrocene::{PREVALIDATED, REQUIRES_VALIDATION};
...
impl<'thir, 'tcx: 'thir> LintThir<'thir, 'tcx> {
    ...
    #[instrument(level = "info", skip(tcx, owner))]
    pub(super) fn check_item(tcx: TyCtxt<'tcx>, owner: OwnerId, item: LocalDefId) -> Option<()> {
        // Check if this is the default function of a trait and is incorrectly marked as
        // `requires_validation` instead of `prevalidated`.
        if tcx.trait_of_assoc(item.into()).is_some()
            && let Some(attr) = tcx
                .get_attrs_by_path(item.into(), REQUIRES_VALIDATION)
                .next()
            && tcx.defaultness(item).has_value() {
            diagnostics::error_incorrect_attr(tcx, attr);
        }
        ...
        if linter.is_none()
            && let Some(trait_assoc_id) = tcx.trait_item_of(item)
            && let Some(attr) = tcx
                .get_attrs_by_path(trait_assoc_id, REQUIRES_VALIDATION)
                .next()
                .or_else(|| {
                    tcx.get_attrs_by_path(trait_assoc_id, PREVALIDATED)
                        .next()
                })
        {
            diagnostics::lint_unvalidated_impl(tcx, item, trait_assoc_id, attr.span());
        }
