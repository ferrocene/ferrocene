...
/// Get the span of the `#[ferrocene::prevalidated]` attribute on `def_id`, if any exists.
pub fn has_validated_attribute(tcx: TyCtxt<'_>, def_id: DefId) -> Option<Span> {
...
pub fn item_is_validated(tcx: TyCtxt<'_>, def_id: DefId) -> ValidatedStatus {
    ...
    let delegation_enabled = tcx.dcx().has_errors().is_some() || tcx.features().fn_delegation();
    let is_main = delegation_enabled || tcx.entry_fn(()).map_or(false, |(main, _)| main == owner);
    ...
    let annotation = has_validated_attribute(tcx, owner);
