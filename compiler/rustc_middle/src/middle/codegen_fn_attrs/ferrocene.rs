...
use rustc_hir::{
    ForeignItem, ForeignItemKind, Item, ItemKind, Node, TraitFn, TraitItem, TraitItemKind, find_attr,
};
...
pub fn item_is_validated(tcx: TyCtxt<'_>, def_id: DefId) -> ValidatedStatus {
    ...
    let annotation = validation_annotation(tcx, owner);
...
fn validation_annotation(tcx: TyCtxt<'_>, item: DefId) -> Option<Span> {
    // find_attr!(tcx, item, a @ Prevalidated(..) => a.span())
    todo!()
    // const PREVALIDATED_ATTR: &[Symbol; 2] = &[sym::ferrocene, sym::prevalidated];
    // const EXPORTED_CONSTRAINT: &[Symbol; 2] = &[sym::ferrocene, sym::exported_constraint];
    // const VALIDATED: &[Symbol; 2] = &[sym::ferrocene, sym::validated];
    //
    // let annotation = tcx.get_attrs_by_path(owner, PREVALIDATED_ATTR).next();
}
