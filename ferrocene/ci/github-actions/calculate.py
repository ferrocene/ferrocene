...
CI_CONFIGURATION = ".github/workflows/test/ci.yml"
...
def calculate_docker_image_tag(platform_plus_image: str):
    ...
    """
    platform, image = platform_plus_image.split("__", 1)
...
def prepare_parameters():
    with open(CI_CONFIGURATION) as f:
        config: dict[str, dict[str, str]] = yaml.safe_load(f)

    replacements: dict[str, Callable[[str], str]] = {
        "docker_image_tag__": calculate_docker_image_tag,
        "docker_image_rebuild__": calculate_docker_image_rebuild,
        "docker_repository_url__": calculate_docker_repository_url,
        "llvm_rebuild__": calculate_llvm_rebuild,
        "targets__": calculate_targets,
    }

    parameters: dict[str, str] = {}
    for parameter in config["env"].keys():
