...
start_vm() {
    ...
    # flags based on the QEMU invocation that `mkqnximage --run`
    # with added shared mount point
    qemu-system-x86_64 \
        -smp 2 \
        -enable-kvm \
        -cpu host \
        ...
        -netdev bridge,br=br0,id=net0 -device virtio-net-pci,netdev=net0,mac="${vm_mac_addr}" \
