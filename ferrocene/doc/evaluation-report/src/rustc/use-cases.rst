...
8. (Optional): If the `user` is building a qualified library, the `user` must verify that only functions the certified subset of libcore are used.
...
10. (Optional): If the `user` is building a qualified executable, the `user` must verify that only functions from the certified subset of libcore are used.
