...
There are three requirements for merging any change into the ``main`` and
...
details, see :ref:`testing:Test Phase 1: Trial Merge and Spot Testing`.

Once active development is done, a PR is opened and this initiates
:ref:`development:Dev Phase 2: Code Review`.


Dev Phase 2: Code Review
^^^^^^^^^^^^^^^^^^^^^^^^
...
command to the bot to queue the PR for :ref:`development:Dev Phase 3: Queue`.
...
:ref:`development:Dev Phase 2: Code Review`. The PR is also removed from the 
queue if any merge conflict arises during this phase to avoid wasting CI time 
with a broken PR.
...
PR goes to :ref:`development:Dev Phase 4: Test`.
...
details, see :ref:`testing:Test Phase 2: Full Testing and Merge`.
...
If the workflow fails, the current PR is moved back to :ref:`development:Dev
Phase 2: Code Review`.
...
change PR. From there, the PR is considered to be in :ref:`development:Dev Phase
2: Code Review`.
