...
* New targets are now supported and qualified for safety critical use. The new targets are:
  ...
  * :target-with-tuple:`aarch64-rhivos2-linux-gnu`
    ...
    * Note: Only when compiled by a :target-with-tuple:`aarch64-unknown-linux-gnu` host

  * :target-with-tuple:`aarch64-unknown-linux-gnu`

    * Note: Only when compiling a :target-with-tuple:`aarch64-rhivos2-linux-gnu` target
