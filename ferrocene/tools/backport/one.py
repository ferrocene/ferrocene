...
def run(*args, **kwargs):
    ...
    proc = subprocess.run(list(args), check=True, **kwargs)
    if VERBOSE:
...
def handle_merge_conflict(pr_name, target_branch):
    ...
    unmerged_solveable = set(
        file.split()[1] for file in unmerged if not file.startswith("UD")
    )
    ...
    print(
        f"NOTE: 'HEAD' in the conflicted files refers to your current branch, {target_branch}"
    )
...
def main():
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Show invoked commands and their outputs"
    )
    parser.add_argument(
        "--force",
        action="store_true",
