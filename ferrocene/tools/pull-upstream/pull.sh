...
#   here, wasting time on resolving lockfiles etc., which we actually want to
#   resolve automatically.
#
#   However, this makes committer information unavailable as well, so we save
#   that into envvars beforehand.
#
# - If, after `git merge` and all the auto-resolutions, there are still conflicts
    ...
    if git diff --diff-filter=U --quiet; then
        ...
        if [ -n "$files" ]; then
            for file in $files; do
                ...
                echo "pull-upstream: trying to resolve conflict for ${file} using mergiraf..."
