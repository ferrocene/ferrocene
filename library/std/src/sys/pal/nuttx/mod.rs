...
pub unsafe fn init(argc: isize, argv: *const *const u8, sigpipe: u8) {
    // HACK something depends on this symbol but nothing defines it because the panic strategy is
    // set to abort. define a stub to avoid program loader errors
    #[no_mangle]
    extern "C" fn __aeabi_unwind_cpp_pr0() {
        unsafe { libc::abort() }
    }

    // if the above symbol cannot be found by the program loader, uncomment this line
    // core::ptr::read_volatile(&(__aeabi_unwind_cpp_pr0 as usize));
    ...
    unsafe fn sanitize_standard_fds() {
        ...
        'poll: {
            #[cfg(not(any( all(target_os = "linux", target_env = "gnu"), target_os = "nuttx")))]
            use libc::open as open64;
            #[cfg(target_os = "nuttx")]
            use libc::open3 as open64;
            ...
            #[cfg(target_os = "nuttx")]
            let mut pfds: [libc::pollfd; 3] = core::mem::zeroed();
            #[cfg(target_os = "nuttx")]
            {
                pfds[1].fd = 1;
                pfds[2].fd = 2;
