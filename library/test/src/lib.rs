...
pub fn test_main(args: &[String], tests: &[&TestDescAndFn]) -> ExitCode {
    let tests = TestList::new(tests, TestListOrder::Unsorted);
    test_main_inner(args, tests, None)
}

fn test_main_inner(args: &[String], tests: TestList<'_>, options: Option<Options>) -> ExitCode {
    let mut opts = match cli::parse_opts(args) {
        Some(Ok(o)) => o,
        Some(Err(msg)) => {
            eprintln!("error: {msg}");
            return ERROR_EXIT_CODE.into();
        }
        None => return ExitCode::SUCCESS, // help was shown
    };
    ...
    if opts.list {
        ...
        match res {
            Ok(true) => {}
            Ok(false) => return ExitCode::from(ERROR_EXIT_CODE),
            Err(e) => {
                eprintln!("error: io error when listing tests: {e:?}");
...
pub fn test_main_env_args_abort(tests: &[&TestDescAndFn]) {
    ...
    if let Ok(name) = env::var(SECONDARY_TEST_INVOKER_VAR) {
        ...
        let mut tests = tests.iter().copied().cloned().collect::<Vec<_>>();
...
pub fn print_merged_doctests_times(args: &[String], total_time: f64, compilation_time: f64) {
    let opts = match cli::parse_opts(args) {
        ...
        Some(Err(msg)) => {
            eprintln!("error: {msg}");
            process::exit(ERROR_EXIT_CODE.into());
        }
        None => return,
    };
...
pub fn filter_tests(opts: &TestOpts, tests: TestList<'_>) -> Vec<TestDescAndFn> {
    let TestList { tests, order } = tests;

    // Initial filtering: Remove tests that don't match the test filter.
    let mut filtered = if opts.filters.is_empty() {
        tests.iter().copied().cloned().collect::<Vec<_>>()
    } else if opts.filter_exact && order == TestListOrder::Sorted {
...
fn filter_exact_match<'a>(tests: &[&'a TestDescAndFn], filters: &[String]) -> Vec<TestDescAndFn> {
