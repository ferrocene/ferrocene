...
pub fn make_tests(config: Arc<Config>, tests: &mut Vec<test::TestDescAndFn>) {
    debug!("making tests from {:?}", config.src_base.display());
    let inputs = common_inputs_stamp(&config);

    let cache = HeadersCache::load(&config);
    let mut poisoned = false;
    collect_tests_from_dir(config.clone(), &cache, &config.src_base, &inputs, tests, &mut poisoned)
...
fn collect_tests_from_dir(
    ...
    find_tests_in_dir(config.mode, dir, &PathBuf::new(), &mut |paths| {
...
fn find_tests_in_dir(
    ...
    if mode == Mode::RunMake && dir.join("Makefile").exists() {
        let paths = TestPaths {
    ...
    for file in fs::read_dir(dir)? {
        ...
        if is_test(&file_name) {
            ...
            if &file_name != "auxiliary" {
                ...
                find_tests_in_dir(mode, &file_path, &relative_file_path, on_test_found)?;
            }
        } else {
