...
pub fn check(root_path: &Path, tidy_ctx: TidyCtx) {
    let dir_path = root_path.join("ferrocene/doc/qualification-report/src/rustc");

    let mut check = tidy_ctx.start_check(CheckId::new("ferrocene_test_outcomes").path(&dir_path));
